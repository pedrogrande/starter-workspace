...
This is the autonomous half of the improvement loop. The user-driven half lives in [`docs/improve-agent.md`](improve-agent.md) (add a tool, add a capability, refine the prompt, fix a specific bug). Use `improve-agent.md` to *change* the agent; use this prompt to *harden* it against its stated intent.
...
- Recommend the user create a feature branch (`git checkout -b tune/<slug>-$(date +%Y%m%d)`) so any wrong turns are easy to revert.
